...
class DosaPage extends StatelessWidget {
  DosaPage({super.key});
...
class RecipeDetPage extends StatelessWidget {
  final FoodItem foodItems;

   RecipeDetPage({
    Key? key,
    required this.foodItems,
    required List<FoodItem> recipes,
  }) : super(key: key);
...
// GoogleFonts.lato
          ...
          Column(
            ...
            children: [
              ...
              Text(
                'Recipe title: ${ foodItems.recipeTitle}',
                style: const TextStyle(
                  fontSize: 23,
                  fontWeight: FontWeight.bold,
                ),
              ...
              const SizedBox(height: 16),
              Text(
                'Cooking Time: ${ foodItems.cookingTime}',
                style: const TextStyle(
                  fontSize: 18,
                  fontWeight: FontWeight.bold,
                ),
              ...
              const SizedBox(height: 16),
              Text(
                'Descriptions:${ foodItems.description}',
                style: const TextStyle(
                  fontSize: 18,
...
class AddToCartPage extends StatelessWidget {
  ...
  const AddToCartPage({Key? key, required this.foodItems}) : super(key: key);

  Future<void> addToCart(BuildContext context, FoodItem recipe) async {
    ...
    try {
      await firestore.collection('cart').doc(documentId).set(foodItems.toMap());
              ...
              Column(
                children: [
                  ...
                  ElevatedButton(
                    ...
                    onPressed: () async {
                      await addToCart(context,  foodItems);
                      // Navigator.push(
                      //     context,
                      //     MaterialPageRoute(
                      //       builder: (context) => CartPage(
      ...
      child: Scaffold(
        body: StreamBuilder<QuerySnapshot>(
          ...
          builder: (context, snapshot) {
            ...
            List<FoodItem> cartItems = snapshot.data!.docs.map((doc) {
              return  FoodItem(
                      ...
                      Row(
                        children: [
                          ...
                          Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              Text( foodItems[index].recipeTitle,style: TextStyle(    
                                fontSize: 17,
                                fontWeight: FontWeight.bold,),),
                              Text(foodItems[index].recipename, style:TextStyle(    
                                fontSize: 16,
                                fontWeight: FontWeight.bold,),),
                            ],
                          ),
